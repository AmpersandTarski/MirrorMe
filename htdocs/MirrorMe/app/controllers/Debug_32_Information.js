/*
Controller for interface "Debug_32_Information" (context: "ShowCritTexts"). Generated code, edit with care.
...
INTERFACE "Debug_32_Information" : '_SESSION'[SESSION] :: SESSION * SESSION  (root interface)
Roles: []
*/
AmpersandApp.controller('Debug_32_InformationController', function ($scope, $rootScope, $route, $routeParams, Restangular, $location, $timeout) {	
	...
	// Create new resource and add data to $scope.resource['Debug_32_Information']
	if($routeParams['new']){
		
		$scope.createResource($scope.resource, 'Debug_32_Information')
			.then(function(data){
				$location.url('/Debug_32_Information/'+ data.content['_id_'], false);
			},function(reason){
				console.log('Failed to create resource: ' + reason);
			});
	
	// Get resource and add data to $scope.resource['Debug_32_Information']
	}else{
		var forceList = true;
		$scope.getResource($scope.resource, 'Debug_32_Information', forceList)
			.then(function(data){
	...
	$scope.newResource = function(){
		$location.url('/Debug_32_Information?new');
	};
	...
	$scope.saveResource = function(resource, requestType){
		...
		resource['_loading_'].push( // shows loading indicator
			Restangular.one(resource['_path_'])
				.patch(resource['_patchesCache_'], {'requestType' : requestType, 'topLevelIfc' : 'Debug_32_Information'})
				.then(function(data) {
					// Update resource data
					if(resource['_ifcEntryResource_']){
						resource['Debug_32_Information'] = data.content;
						//tlResource = resource;
					}
