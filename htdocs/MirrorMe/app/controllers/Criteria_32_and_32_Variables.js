/*
Controller for interface "Criteria_32_and_32_Variables" (context: "ShowCritTexts"). Generated code, edit with care.
Generated using template: controllers/controller.js
Generated by Ampersand v3.5.0[branch_#228:44da3d6], build time: 08-Apr-16 08:36:09 West-Europa (zomertijd)

INTERFACE "Criteria_32_and_32_Variables" : '_SESSION'[SESSION] :: SESSION * SESSION  (root interface)
Roles: []
*/
AmpersandApp.controller('Criteria_32_and_32_VariablesController', function ($scope, $rootScope, $route, $routeParams, Restangular, $location, $timeout) {	
	...
	if($routeParams['new'] && 'SESSION' == 'SESSION') resourceId = '_NEW_'; // Set resourceId to special '_NEW_' value in case new resource must be created 
	$scope.resource = Restangular.one('resources').one('SESSION', resourceId); // BaseURL to the API is already configured in AmpersandApp.js (i.e. 'http://pathToApp/api/v1/')
	$scope.resource['_path_'] = '/resources/SESSION/' + resourceId;
	$scope.resource['_ifcEntryResource_'] = true;
	
	// Create new resource and add data to $scope.resource['Criteria_32_and_32_Variables']
	if($routeParams['new']){
		
		$scope.createResource($scope.resource, 'Criteria_32_and_32_Variables')
			.then(function(data){
				$location.url('/Criteria_32_and_32_Variables/'+ data.content['_id_'], false);
			},function(reason){
				console.log('Failed to create resource: ' + reason);
			});
	
	// Get resource and add data to $scope.resource['Criteria_32_and_32_Variables']
	}else{
		var forceList = true;
		$scope.getResource($scope.resource, 'Criteria_32_and_32_Variables', forceList)
			.then(function(data){
	...
	$scope.newResource = function(){
		$location.url('/Criteria_32_and_32_Variables?new');
	};
	...
	$scope.saveResource = function(resource, requestType){
		...
		resource['_loading_'].push( // shows loading indicator
			Restangular.one(resource['_path_'])
				.patch(resource['_patchesCache_'], {'requestType' : requestType, 'topLevelIfc' : 'Criteria_32_and_32_Variables'})
				.then(function(data) {
					// Update resource data
					if(resource['_ifcEntryResource_']){
						resource['Criteria_32_and_32_Variables'] = data.content;
						//tlResource = resource;
					}
